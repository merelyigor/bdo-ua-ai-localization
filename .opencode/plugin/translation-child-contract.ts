...
export const TranslationChildContract: Plugin = async ({ directory }) => {
  ...
  "tool.execute.after": async (input, output) => {
    ...
    output.output = [
      ...
      // Формулювання навмисно спокійне й наказове.
      //
      // Перша версія цього рядка починалась із «УВАГА: твій аргумент
      // проігноровано». Диригент прочитав це як вимогу дати правильний вміст,
      // почав пояснювати власнику, що не має копії staged файла, і завис на
      // місці: три «продовжуй» поспіль без жодного кроку (пачка
      // 20260829_222317, 2026-08-29). Сигнал мусить знімати роботу, а не
      // додавати її.
      ignored
        ? "Payload підставив набір · це нормально, дані цілі. Наступного разу просто передай рядок next.prompt із відповіді ./bdo run drive; збирати вміст не треба."
        : "",
