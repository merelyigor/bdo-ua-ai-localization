...
declare -A MAX_LINES=(
    ...
    # `no_current_batch`. 2026-08-27: 135 -> 150, бо додано розпізнавання
    ...
    # здогадів. Відповідність дванадцяти назв не стискається без втрати сенсу,
    # а без неї слабка модель або перепитує, або вигадує категорію.
    [патч]=150 [ручний]=150 [пропозиції]=150 [покращення-ші]=150
)
