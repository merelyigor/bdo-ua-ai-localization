...
declare -A MAX_LINES=(
    ...
    # КАТЕГОРІЙ: власник просить «покажи по категоріях» і «переклади квести»
    # живою мовою, а модель мусить перевести це в англійський `domain` без
    # здогадів. Відповідність тринадцяти назв не стискається без втрати сенсу,
    # а без неї слабка модель або перепитує, або вигадує категорію.
    #
    # `покращення-ші` довший за решту на дев'ять рядків навмисно: лише він
    # пояснює, ЩО саме робить режим (переклад наново спадщини Bosia з
    # англійського джерела), і без цього пояснення модель бере `active` замість
    # патча, де лежить робота.
    #
    # 2026-08-27, друга правка: 155 -> 165 лише для `покращення-ші`. Локальна
    # модель на питання «що є на покращення?» пішла читати `docs/plans/BACKLOG.md`
    # замість рядків, бо режим не мав ні джерела числа, ні правила розрізнення.
    # Обидва тепер у промпті; решта режимів лишається на 155.
    #
    # 2026-08-28: 155 -> 170 і 165 -> 180. Додано два правила, без яких власник
    # мусив вручну випрошувати звіт: обовʼязковий текстовий звіт про будь-яку
    # відмову і пряма заборона лагодити код набору (диригент двічі намагався
