...
- **`git push` робить ВИКЛЮЧНО власник.** Агент не пушить ніколи · ні за дозволом,
  ні за проханням. Те саме для force-push, тегів і будь-якої зміни remote. Агент
  лише повідомляє, що готове до push, і називає гілку. Деталі · §4.2.
- **`git commit` агент робить тільки коли власник попросив або дав згоду** на
  запит агента в цьому ж діалозі. Дозвіл на один коміт не переноситься на
  наступні. Деталі · §4.3.
- force/rebase/tag/amend і зміна історії · лише за одноразовим явним дозволом.
  AI-атрибуція у файлах і commit messages заборонена.
...
- `BDO_ENV` є і ціллю, і дозволом: окремої фрази на прод-запис агент не просить.
  Свідомою дією власника є правка `.env`, а не повторення дозволу в чаті. Перед
  першою пачкою агент ОГОЛОШУЄ ціль одним рядком · це шанс перебити, не запит.
  Запис відбувається лише коли прохання означає «перекласти»; «перевір»,
...
- Push · дія власника, не агента (§4.2). Коміт · на прохання або за згодою (§4.3).
- Commit message після дозволу · тільки за `.codex/prompts/commit-message-ua.txt`;
  версія зростає, жодних trailers або AI-атрибуції. Backticks навколо
  ідентифікаторів дозволені, markdown-розмітка · ні. Hooks path · `.githooks`.
