...
- Виняток для native child-сесій `translation-*`: вони не готують власнику
  звіт і не застосовують формат `## Звіт`/`## Що далі`; їхній єдиний контракт ·
  окремий prompt та JSON-схема ролі.
- Фінал основного агента має два блоки: `## Звіт`, останнім `## Що далі`.
  Це формат для власника як для людини, не технічний протокол.
- `## Звіт`: **Зроблено** 2-5 речень простою мовою, не журнал дій -> **Зʼясовано**
  лише висновки, що впливають на рішення власника, кожен із доказом; таких немає ·
  блок пропустити -> **Перевірки**, пропорційні зміні, з командами, результатами,
  пропусками та ризиками -> `SEO-check` -> `OSM-check`. Виправлення власної хибної
  заяви · окремий рядок.
- `## Що далі` при повністю досягнутій меті · рівно один рядок «Мету досягнуто,
  додаткова робота не потрібна.» Інакше · що лишилось, наступний конкретний крок,
  потреба від власника, Done criteria; кожен пункт із маркером `[потрібно]` або
  `[опційно]`. Вигадані варіанти й перенесення у «Що далі» роботи з поточної задачі
  заборонені. Аналіз над звітом короткий; звіт фіксує результат, не процес.
