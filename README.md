...
**Робочою лишається GGUF-збірка** `qwen3.6:35b-a3b-mtp-q4_K_M`. Виміряно
2026-09-05 через `./bdo bench` на ОДНАКОВИХ payload живої пачки, по 14 теплих
викликів на модель (перше завантаження ваги рахується окремо):

| збірка | вихідних токенів/с | схема ok |
|---|---|---|
| `qwen3.6:35b-a3b-mtp-q4_K_M` (GGUF) | 46.1 | 14/14 |
| `qwen3.6:35b-mlx` (MLX) | 34.9 | 13/14 |

Перевага GGUF тримається на всіх трьох ролях (45-46 проти 27-39 токенів/с).
Порівнювались саме ці дві ЗБІРКИ, а не формати взагалі: прискорення може
давати `mtp` (multi-token prediction) у тезі GGUF, а не GGUF як такий.

Одна серія нічого не доводить · перша, коротша, дала 41.7 проти 23.2 лише тому,
що щойно завантажена MLX-модель платила холодний старт (32.6 с проти 15.4 с).
Тому вимір повторюваний: `./bdo bench --capture` під час пачки, далі
`./bdo bench <тег> <тег>` · без жодного запису в API. Якість самого тексту цим
не міряється: для неї потрібні живі пачки й квитанції.
