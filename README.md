...
| Моделі субагентів | [Ollama](https://ollama.com) з моделлю зі списку профілю (GGUF або MLX) |
...
За тестами власника проєкту, для режимів `Патч`, `Покращення ШІ`, `Пропозиції` та
...
`патч`, `ручний`, `пропозиції` та `покращення-ші`. Останній перекладає НАНОВО
рядки ШІ-шару, зроблені з російського reference: джерелом істини є англійський
текст, російський іде моделі лише як підказка про сенс, а вибірку звужує
серверний фільтр `machine_provenance=legacy`. `build`, `plan`, `general`
...
- Відкрийте меню режимів і виберіть потрібний primary-режим: `Патч`,
   `Покращення ШІ`, `Пропозиції` або `Ручний`.
