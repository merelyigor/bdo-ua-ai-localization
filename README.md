...
![BDO UA AI Localization · чат у OpenCode, пʼять локальних субагентів, механічні перевірки, запис через Agent API](docs/assets/banner.png)
...
викликаючи **локальних субагентів** на безкоштовних моделях у Ollama і
...
- Робота починається в чаті одним реченням: «переклади 15 рядків активного
  патча». Агент `translation` уже вибраний за замовчуванням, і всю пачку він
  виконує однією командою `./bdo batch run`.
...
рядка: адресу API правити не треба, вона зашита в `select-env.sh` як публічна
...
Перебивати адресу потрібно лише при self-hosting: `BDO_API_BASE_PROD=...` або
`BDO_API_BASE=...`. Значення `BDO_ENV=DEV` призначене тим, хто розробляє сам
проєкт BDO UA Translate, і вимагає власного `BDO_API_BASE_DEV` у приватному
`.env` · для перекладу воно не потрібне.
...
Лікування · рівно одне коло, і воно всередині `./bdo batch run`:
...
закрили. Другого кола не буває: у ШІ-шар рядок іде як є після repair, у ручному
прогоні · у модерацію. Гонитва за 100% PASS одного разу зʼїла 11 сесій на
20 рядків.
...
| `proposal` | черга модерації | уся пачка на розгляд людини |
...
- `manual` і `proposal` · планка діє: чисте йде з автоапрувом за роллю, справді
  проблемне (`major`, `critical`, `REJECT`) · у пропозиції, бо ручний шар і є
  людська правда.
...
.opencode/                  усе, що вантажить OpenCode
  agents/translation.md       промпт диригента
...
*.sh                        реалізація підкоманд (прямий виклик теж працює)
