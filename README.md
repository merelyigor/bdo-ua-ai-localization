...
Думання є явним налаштуванням виклику. Довгі роздуми дозволені; потік
зупиняється лише за повторюваним фрагментом thinking або останнім аварійним
рубежем часу. `content` і `thinking` приходять окремо.

## Запуск на трьох ОС

- Windows · подвійний клік по `bdo.bat` запускає набір нативним PHP; WSL2 лишається запасним способом · див. [`docs/WINDOWS_WSL2.md`](docs/WINDOWS_WSL2.md).
- macOS · подвійний клік по `BDO.app` піднімає інтерфейс і відкриває браузер.
- Linux · один раз виконай `./bdo desktop --install`, потім запускай набір ярликом у меню програм.
...
обидві 6/6 під strict-схемою. Різниця 7% · у межах шуму.

(Перший вимір давав 46.1 проти 34.9 і виглядав переконливо. Він був хибний:
у памʼяті лежали ОБИДВІ збірки, 23 + 33 ГБ на 69 ГБ RAM, і крок QA, який
звичайно триває 55-142 с, одного разу не завершився й за 11 хвилин. Тому
`./bdo bench` тепер сам вивантажує чужу модель перед виміром · це єдине, що він
робить із Ollama, і робить лише на час виміру.)

Вирішили живі пачки 2026-09-06:

| | пачок | рядків | у шар | до людини | карантин | викликів на пачку |
|---|---|---|---|---|---|---|
| GGUF `35b-a3b-mtp-q4_K_M` | 7 | 350 | 288 (82%) | 59 (17%) | 3 (0.9%) | 5.7 |
| MLX `35b-mlx` | 3 | 150 | 45 (30%) | 4 (3%) | 101 (67%) | 10.0 |

Причину назвав сам конвеєр: `qa_incomplete` · «QA повернув 44 вердиктів на
50 рядків». На payload робочого розміру MLX-збірка систематично віддає НЕ ВСІ
рядки під strict-схемою; драйвер повторює виклик (звідси 10 викликів на пачку
проти 5.7), бюджет спроб вичерпується, і пачка йде в карантин цілком. Дві
пачки поспіль дали 0 записаних рядків із 50. Контрольна перевірка: ту саму
пачку, яку MLX кинув, GGUF закрив 50 із 50, наступна дала 48 із 50.

Рядки при цьому не втрачені · сервер віддає їх у наступну вибірку, доки вони
не записані.

На payload із 5 рядків бенчмарка цього не бачила взагалі (6/6 повних
відповідей): дефект вилазить саме на робочому розмірі пачки. Тому
`./bdo bench --capture` тепер ВІДМОВЛЯЄТЬСЯ зберігати замалий знімок
(`BDO_BENCH_MIN_ROWS`, типово 25) і каже, коли його знімати · на кроці QA.
На фікстурі з 36 рядків відрив одразу більший: 48.6 проти 33.9 токенів/с.
Але й це не заміна живій пачці · придатність моделі доводить лише вона.

Порівнювались саме ці дві ЗБІРКИ · і це ПРАВИЛЬНЕ порівняння для нашого
вибору. У бібліотеці Ollama для `qwen3.6:35b` збірки MLX із `mtp` не існує
(є `35b-mlx`, `35b-a3b-mlx-bf16` 72 ГБ, `mxfp8` 38 ГБ, `nvfp4`), а GGUF ми
беремо саме з `mtp`. Тому питання «GGUF чи MLX» на практиці означає «`mtp` чи
без нього», і відповідь на нього дає рівно цей вимір.

Повторити: `./bdo bench --capture` під час пачки, далі `./bdo bench <тег> <тег>`
· без жодного запису в API.
...
У Windows те саме дає подвійний клік по `bdo.bat` (нативний PHP, WSL2 · запасний
спосіб), на macOS · значок `BDO.app` (його можна перетягнути в Dock: перший клік підіймає
інтерфейс і відкриває браузер, наступний питає «відкрити» чи «зупинити»), з
IDE · `make web`. Значок мусить лишатися в теці набору: Dock тримає посилання
на оригінал, тому перетягувати можна, а копіювати кудись інде · ні.

Значок `BDO.app` носить своє обличчя (`Contents/Resources/BDO.icns`). У Windows
так не вийде: `.bat` не може нести значка · його показує ЯРЛИК. Правий клік по
`bdo.bat` → «Створити ярлик» → «Властивості» → «Змінити значок» → вибрати
`bdo.ico` з теки набору; ярлик далі перетягується на панель задач.

Далі набір працює сам: на сторінці видно кожну роль, потік
...
(`model_unreachable`, `empty_content`, `thinking_loop`, `truncated`, `not_json`,
