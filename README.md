...
| `manual` | ручний шар, схвалення за роллю; проблемне автоматично в proposal | «як людина» із запобіжником |
| `proposal` | черга модерації | окремий режим: уся пачка на розгляд людини |
...
- `manual` · чисте йде як ручна пропозиція з `auto_approve=true`, тому сервер
  схвалює її відповідно до ролі власника; справді проблемне (`major`, `critical`,
  `REJECT`) примусово йде окремою пропозицією з `auto_approve=false`.
- `proposal` · окремий режим, у якому вся непорожня пачка одразу йде на
  модерацію з `auto_approve=false`, незалежно від QA-статусу.
