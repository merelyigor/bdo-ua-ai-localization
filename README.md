...
- спочатку диригент готує пачку й визначає, які child-ролі потрібні;
- `translation-worker` створює кандидат перекладу для кожного рядка;
- `translation-qa` перевіряє кожен кандидат і повертає формальний вердикт;
- `translation-judge` вирішує, у який канал має потрапити спірний рядок.
...
**`translation-worker` — перекладач пачки.** Він отримує підготовлені рядки та
створює лише кандидатські переклади. Його завдання — не вирішувати, куди писати
результат, а повернути структурований текст для наступних перевірок.

![Child-сесія translation-qa](docs/assets/screenshots/05-translation-qa.png)

**`translation-qa` — контроль якості.** Він запускається після worker і окремо
перевіряє кожен рядок: збереження identity, placeholders, markup, довжини,
мовної якості та інших механічних обмежень. Його результат — вердикт, а не новий
довільний переклад.

![Child-сесія translation-judge](docs/assets/screenshots/06-translation-judge.png)

**`translation-judge` — маршрутизатор спірних випадків.** Він не переписує текст,
  а визначає, що робити з рядком після QA: залишити в `ai_layer`, передати в
  `manual` або відправити в `proposal` на модерацію. Це захищає flow від тихого
  запису сумнівного результату не в той канал.
...
.opencode/                  усе, що вантажить OpenCode
  ...
  agents/translation-*.md     шість субагентів: роль, модель, права, інструменти
