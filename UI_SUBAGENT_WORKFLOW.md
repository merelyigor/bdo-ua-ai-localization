...
3. Плагін проєкту відхиляє будь-який маршрут, відсутній в активній політиці
   ролі. Платні маршрути вимагають `allow_paid=true`; дотримання схеми моделлю
   доводить `./bdo runtime`, а не назва тега.
...
7. `translation-child-contract` робить доставку payload механічною: на native
   ...
   `response_path`. Відповідь приходить у strict-обгортці · `{"items":[…]}` для
   всіх ролей, бо structured outputs в OpenAI-сумісних провайдерів вимагають
   обʼєктний корінь; плагін розгортає її назад у масив, тому жоден скрипт нижче
   за течією обгортки не бачить. Слабкий диригент не є надійним носієм байтів: обрізана копія
...
8. Дефект формату відповіді не зупиняє ланцюжок. Якщо весь текст відповіді є
   ...
   Успішна відповідь знімає позначку; `BDO_CHILD_RETRY_WINDOW_SECONDS` (типово
   600 секунд) задає backoff до 60 секунд, а driver сам відкриває наступні
   вікна. Лише `BDO_CHILD_RETRY_TOTAL_SECONDS` (типово 86400) завершує повтори
   для тієї самої ролі пачки. Розбір потім · `./bdo incidents`.
   `translation_result` вважає захоплений файл канонічним і використовує
...
9. `translation-judge` вирішує лише МАРШРУТ спірного рядка (у ШІ-шар як є чи до
   ...
   вимикається `BDO_JUDGE=off`, поріг · `BDO_JUDGE_MIN_CONFIDENCE` (типово 65).
...
Big Pickle не оголошує перелік effort у каталозі, тому для нього діє безпечний
fallback плагіна без вигаданого значення, а provider сам застосовує свій default.
...
(лише за потреби) -> commit. Для тимчасових помилок child діють короткі
retry-вікна з backoff у межах 24-годинного загального бюджету; пачка не стає
постійно заблокованою.
...
Патч обирається третім аргументом: `./bdo mode start patch 50 3` бере знімок 3
...
`identity_hash`, хоча `enum` схеми робить це неможливим, поки схема діє.
...
- Для роботи з пачками використовуйте один із чотирьох primary-режимів (`патч`,
  `ручний`, `пропозиції`, `покращення-ші`). Їхні правила живуть у власних
