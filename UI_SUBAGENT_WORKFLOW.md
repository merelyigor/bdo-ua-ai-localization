...
8. Дефект формату відповіді не зупиняє ланцюжок. Якщо весь текст відповіді є
   ...
   Успішна відповідь знімає позначку; повтори мають часовий бюджет
   `BDO_CHILD_RETRY_WINDOW_SECONDS` (типово 600 секунд) і backoff до 60 секунд.
   Після вікна drive повертає `retry`, не постійний block; наступний виклик
   відкриває нове вікно для тієї самої пачки. Розбір потім · `./bdo incidents`.
...
(лише за потреби) -> commit. Для тимчасових помилок child діє спільне
часове retry-вікно з backoff; пачка не стає постійно заблокованою.
