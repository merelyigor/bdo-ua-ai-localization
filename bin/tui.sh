...
target() {
    ...
    # Вікно бачить власник, а не розробник: порада «виконай ./bdo env» тут
    # безадресна · він команд не складає. Кажемо, ДЕ саме ціль задається.
    printf '%s' "${found:-невідома · ціль задає рядок BDO_ENV у файлі .env}"
}
