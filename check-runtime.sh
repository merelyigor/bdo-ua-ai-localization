...
MODEL="${MODEL_FULL#ollama-local/}"
test -n "$MODEL" || { echo "FAIL: у opencode.json немає моделі translation-worker" >&2; exit 1; }
...
curl -fsS -m 5 "$OLLAMA_URL/v1/models" 2>/dev/null \
    ...
    || { echo "FAIL: $OLLAMA_URL не відповідає або моделі $MODEL немає (ollama pull $MODEL)"; exit 1; }
echo "OK"
...
if "$SCRIPT_DIR/sync-opencode-models.sh" >/dev/null 2>&1; then
    echo "OK"
else
    echo "FAIL"
    "$SCRIPT_DIR/sync-opencode-models.sh" || true
    exit 1
fi
