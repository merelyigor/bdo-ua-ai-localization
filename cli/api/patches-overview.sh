...
// Статус патча українською. `superseded` навмисно перекладено як «неактивний»,
// а не «закритий» чи «застарілий»: диригент 2026-08-27 прочитав англійське
// слово як «дані вже неактуальні для перекладу» і оголосив, що працювати можна
// лише в активному патчі. Насправді саме в неактивних лежить уся робота ·
// 29 920 рядків без ШІ-шару в патчі 1. Слово має означати рівно «не активний
// зараз», без натяку на те, що там нема чого робити.
$ua = ["superseded" => "неактивний", "active" => "активний", "draft" => "чернетка",
       "imported" => "імпортований", "archived" => "архівний"];
foreach ($d["data"]["patches"] ?? [] as $p) {
    printf("%s\t%s\t%s\t%s\t%s\t%s\t%s\t%s\n",
        ...
        ($p["is_active"] ?? false) ? "активний" : $ua[(string) ($p["status"] ?? "")] ?? (string) ($p["status"] ?? ""));
}')"
...
echo "«неактивний» НЕ означає «неактуальний»: саме там і живе вся робота.\n";
echo "Узяти патч у роботу: сторінка ./bdo web або меню ./bdo\n";
