...
# не вилікувалось, іде в модерацію, де його подивиться людина. Виняток один і
# він названий у коді нижче: рядок із НАКАЗОВОЮ підказкою API («ужий «X» для
# «Y»») отримує одну додаткову спробу, бо для нього «не вилікувалось» означає
# карантин, а не модерацію.
#
...
foreach (array_keys($defects) as $hash) {
    ...
    // Одне коло · для вироків QA, і це виміряне рішення 2026-08-16: смакові
    // зауваження друге коло майже не рятує, а коштує хвилин.
    //
    // Відмова API з явним `expected` · інша природа. Це не думка про стиль, а
    // точна вказівка «ужий «Човен» для «Ship»», і виконати її модель або може,
    // або ні. Ціна помилки теж інша: такий рядок не потрапляє НІКУДИ · канал
    // модерації перевіряє глосарій тим самим правилом і теж відмовляє, тому
    // рядок падає в карантин. Заміряно 2026-09-04 на пачці `20260904_055351`:
    // 15 рядків із 50 (30%) згоріли саме так при одному колі.
    //
    // Тому таким рядкам дається ОДНА додаткова спроба, і лише поки підказка
    // справді наказова (у ній є «ужий «…» для»). Смакові вироки залишаються на
    // одному колі, як і було.
    $actionable = array_filter(
        $defects[$hash],
        // Лапки тут ПОДВІЙНІ, і це не стиль: увесь цей PHP лежить усередині
        // php -r у одинарних лапках оболонки, тому одинарна лапка закрила б
        // рядок і зламала скрипт.
        static fn (string $d): bool => str_contains($d, "ужий «") || str_contains($d, "збережи токен «"),
    ) !== [];
    $limit = $actionable ? $maxAttempts + 1 : $maxAttempts;
    $done = (int) ($attempts[$hash] ?? 0);
    if ($done >= $limit) { $hopeless[$hash] = $done; continue; }
