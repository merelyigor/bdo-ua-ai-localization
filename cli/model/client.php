...
// сама змінна `$argv` існує лише при увімкненому `register_argc_argv`, тому
// код, який читає її напряму, спирається на налаштування рантайму замість
// факту. Набір возить портативний PHP на Windows · там `php.ini` чужий.
...
// СТЕЛЯ ПО ЧАСУ Є, І ВОНА ЩЕДРА · рішення власника 2026-09-19.
//
// Це НЕ бюджет якості й не межа думання: модель має думати стільки, скільки їй
// треба, а зациклення ловить детектор повторів. Але безрезультатна робота без
// кінця теж нікому не потрібна, тому лишається останній рубіж · 1500 с.
// Попередні 900 с виявились замалими: на пачці `20260919_011019` термінолог
// думав ~157 с, далі рантайм замовк, і виклик обірвався на 1057-й секунді.
//
// `0` лишається значенням «не обривати взагалі» на випадок свідомого рішення.
// Технічно нуль у контексті `http` дорівнював би `default_socket_timeout`
// (60 с), тобто ЖОРСТКІШІЙ межі · саме тому нуль перекладається у велике
// число, а не передається як є.
$configuredTimeout = (int) ($config['timeout_seconds'] ?? 1500);
...
$publishLiveUsage = static function () use (&$liveThinkingText, &$liveAnswerText, &$thinkingTokensEstimate, &$answerTokensEstimate, &$liveTokenEstimate, $callMarker, $relative, $payloadPath, $role, $model, $provider): void {
    ...
    'model' => $model,
    'provider' => $provider,
    // ЯКИЙ САМЕ PAYLOAD ПОЇХАВ У МОДЕЛЬ. Сторінка показувала «запит поточного
    // виклику», вибираючи НАЙСВІЖІШИЙ `*-payload.json` у теці пачки · тобто
    // вгадувала. 2026-09-19 суддя думав над 1487 байтами, а власник бачив
...
$partialItems = null;
if ($done !== 'stop' && $content !== '') {
    // ОБІРВАНА ВІДПОВІДЬ · не порожня. Модель пише масив елемент за елементом,
    // і всі, які вона ДОПИСАЛА до кінця, цілі й правильні. Доти набір викидав
    // усю відповідь: дванадцять готових виправлень зникали разом із тринадцятим
    // недописаним, і пачка починалась спочатку.
    //
    // Недописаний хвіст відрізається, ніщо не дописується, а рядки, яких не
    // прийшло, лишаються невиконаними · повноту там, де вона критична, і далі
    // вимагає драйвер (`./bdo items --require-all`).
    require_once __DIR__.'/unwrap.php';
    $partialItems = bdo_partial_child_json($content);
}
if ($partialItems !== null) {
    $jsonSalvage = sprintf('обрив (%s): узято цілих елементів %d', $done, count($partialItems));
    fwrite(STDERR, 'УВАГА: відповідь обірвана · '.$jsonSalvage.", недописаний хвіст відкинуто\n");
} elseif ($done !== 'stop') {
...
$decoded = $partialItems ?? json_decode($content, true);
if (! is_array($decoded)) {
