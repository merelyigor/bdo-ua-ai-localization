...
if [ "${BDO_ORCHESTRATOR:-php}" != sh ]; then
    exec php "$(cd "$(dirname "$0")/../.." && pwd)/cli/bdo.php" names-payload "$@"
fi
...
foreach ($validate?->results() ?? [] as $result) {
    ...
    $item = [
        "identity_hash" => $hash,
        // АНГЛІЙСЬКОГО ДЖЕРЕЛА ТУТ НЕМАЄ · це друга економія проходу після
        // глосарія, і вона ж захист від зайвої роботи.
        //
        // Заміряно 2026-09-05 на пачці `20260905_210828`: payload на 3 рядки
        // важив 11.5 КБ, і майже все це · ДВІ копії довгого опису предмета,
        // англійська й українська, там де треба замінити одну назву.
        // Канонікал, який шукаємо, уже стоїть у наказі («ужий «X» для «Y»»),
        // тому джерело не додає жодного факту. І головне: без джерела роль не
        // може «перекласти наново», а саме цього вона робити НЕ повинна.
