...
if [ "${BDO_ORCHESTRATOR:-php}" != sh ]; then
    ...
    exec php "$SCRIPT_DIR/cli/bdo.php" build-items "$@"
fi
...
# `cli/quality/build-items.sh rows.json clean.json --require-all` мовчки створював у теці
...
# Вихід cli/runtime/model-ab.sh - вимір, а не переклад: він народився поза видимою
# субагентською сесією. Механічна відмова тут і в cli/batch/batch-commit.sh - це те, що
