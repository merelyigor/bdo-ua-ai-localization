...
completion() {
    ...
    //
    // Вона рахувала, скільки payload осіло в транскрипті платної моделі, і
    // радила відкрити нову сесію режиму. Ні транскрипту, ні сесії більше
    // немає: драйвер тримає стан на диску, а модель бачить рівно один payload
    // за виклик. Лічильник `session-load.json` не пише ніхто, тому підказка
    // спрацьовувала кожні дві пачки з нулем у КБ і радила дію, якої не існує.
