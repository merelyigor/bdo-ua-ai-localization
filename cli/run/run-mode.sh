...
if [ "$count" -eq 0 ]; then printf '{"ok":true,"mode":"%s","patch":"%s","state":"complete","rows":0}\n' "$MODE" "$PATCH"; exit 0; fi
...
$ledger["batches"] = ($ledger["batches"] ?? 0) + 1;
file_put_contents($file, json_encode($ledger, JSON_UNESCAPED_UNICODE));
