...
if [ "${BDO_ORCHESTRATOR:-php}" != sh ]; then
    exec php "$(cd "$(dirname "${BASH_SOURCE[0]}")/../.." && pwd)/cli/bdo.php" web "$@"
fi
...
# Воркерів 8, а не 4. Кожне SSE-зʼєднання займає один воркер, і кожен запуск
# `./bdo web` відкриває НОВУ вкладку · чотирьох не вистачало вже на четвертій:
# звичайний запит чекав 10.5 с (заміряно 2026-09-05, D76). Вісім дають запас
# навіть якщо частина вкладок не встигла відпустити зʼєднання.
WORKERS="${BDO_WEB_WORKERS:-8}"
