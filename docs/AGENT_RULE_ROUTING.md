...
- Зміна контракту ролі оновлює три речі разом: `roles/<роль>.md`, схему в
  `lib/Cli/Command/Prepare/BuildSchemaCommand.php` і вузьку перевірку в
  `tests/`. Однойменний `cli/prepare/build-schema.sh` уже є ЗАМОРОЖЕНОЮ
  оболонкою зі шляхом відкату (`BDO_ORCHESTRATOR=sh`): правка в ньому на
  живому прогоні нічого не змінить.
