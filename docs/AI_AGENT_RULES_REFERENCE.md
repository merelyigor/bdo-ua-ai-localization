...
- §8.9 Кожен runtime primary/child prompt є повним і самодостатнім. Заборонені
  ...
  `.md`, generator deterministic, а gate звіряє результат із джерелом. Доки
  генератора немає, спільний блок чотирьох primary-промптів тримає до символу
  `tests/primary-prompt-drift.sh`: без нього посилення доїжджає в один режим
  із чотирьох.
...
- §8.12 Еталонна «найслабша модель» названа: `ollama-local/qwen3.6:35b-a3b-mtp-q4_K_M`
  (курс власника на локальні моделі, 2026-08-27). Основна вага · CHILD-ролі:
  70-80% дитячих сесій ідуть локально, тоді як primary зазвичай працює на
  сильнішій моделі. Child-prompt пишеться під локальну модель, а мовний зрив
  (чужа писемність, русизм, гомогліф) ловиться детектором у `lib/Quality/**`,
  бо промпт лише просить. Заміряно того ж дня: у ролі диригента вона
  за шість повідомлень порушила три вже наявні правила · перемалювала вивід
  `./bdo patches all --full` у власну таблицю, подала стан `none` як «без
  ШІ-шару» і вигадала команду `./bdo status`. Висновок нормативу: правило, яке
  не має механічної перевірки, для локальної моделі є побажанням. Кожне нове
  правило промпта закривається grep-перевіркою в `scripts/agent-check.sh` або
  окремим тестом.
- §8.13 Child-prompt задає рамку задачі: офіційна українська локалізація Black
  Desert Online, а рядок · назва предмета, опис, квест, вміння, інтерфейс або
  системне повідомлення. Причина: до 2026-08-28 рамки не було взагалі, гра
  згадувалась лише як заборона перекладати токен `Black Desert`, і модель не
  мала підстав задіяти знання про гру з навчання. Межа обовʼязкова разом із
  рамкою: знання гри пояснює СЕНС, відповідник дають `glossary`, `terms`,
  `examples`, а спогад про чужу локалізацію (насамперед російську) не
  переноситься · саме звідти беруться русизми, які ловить `lib/Quality/**`.
  Наявність рамки й межі перевіряє `.opencode/validate-translation-agents.sh`.
...
- §12.1 Перевірка мусить проходити ТИМ САМИМ шляхом і тим самим артефактом, що й
  ...
  показував зелений маршрут на конфігурації, де жоден реальний субагент не
  працював: `opencode-go` мав 0 успішних дитячих сесій із 3. Перевірка, яка не
...
- §12.6 Виправлення мусить доходити до вже запущеної роботи. OpenCode вантажить
  промпти й плагіни один раз на старті, а staged-схема будувалась один раз на
  пачку · тому виправлений формат не діяв ні в старій сесії, ні в пачці, що вже
  висіла. Артефакт або перебудовується перед кожним використанням, або входить у
  `WORKFLOW_FILES` restart-gate.
- §12.7 Симптом «модель падає» вимагає розрізнення в коді, а не в чаті. Дитяча
  сесія з нулем токенів і без інциденту формату означає, що запит не дійшов до
  моделі; це конфігурація, а не тимчасовий збій, і чекати на неї добу не можна.
  Механічну межу тримає `BDO_CHILD_SILENT_LIMIT` і причина `child_no_response`.
- §12.8 Джерело правди про факт роботи · сервер, а не звіт агента. Після пачки
  перевіряти залишок через `./bdo patch <N>`, а поведінку субагентів · у базі
  OpenCode. Формулювання «пачка успішна» без цифри з API не приймається.
