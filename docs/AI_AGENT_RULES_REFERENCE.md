...
- §10.5 Фінал має рівно два блоки: `## Звіт`, потім `## Що далі`. **Зроблено** ·
  2-5 речень простою мовою, не журнал дій. **Зʼясовано** · лише висновки, що
  впливають на рішення власника, кожен із доказом; таких немає · блок пропустити.
  **Перевірки** пропорційні зміні: команди, результати, пропуски й ризики. Далі
  `SEO-check` та `OSM-check`. Виправлення власної хибної заяви · окремий рядок.
- §10.6 `## Що далі` при повністю досягнутій меті · рівно один рядок «Мету
  ...
  Для цього CLI/docs repo `SEO-check` та `OSM-check` зазвичай `N/A`, але мають бути
  названі.
