...
- §4.2 Заборонені без одноразового явного дозволу: commit, push, force, rebase,
  tag, amend, зміна історії або remote state. Не використовувати `git add -A`,
  `git add .`, `git commit -a`.
- §4.3 Commit message після дозволу строго відповідає
  `.codex/prompts/commit-message-ua.txt`, має зростаючу версію та не містить
  trailers, AI attribution або generated-by text.
- §4.4 Перед видаленням state/output, переписуванням історії, production write чи
  іншою необоротною дією назвати, що зникне, як відновити і що перевірити після.
- §4.5 `output/`, `state/`, `state-auto/` містять audit/cursors/quarantine.
  `batch-clean.sh` без `--apply` є preview; автоматично ці теки не видаляти.
