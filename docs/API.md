...
Ігрового номера патча й дати випуску API не віддає: у `meta` є рівно
`snapshot_id`, окремого переліку патчів немає (`/patches`, `/snapshots`,
`/patch/list` дають 404, у `/guide` такого ендпоінта теж немає). Тому нумерація
в наборі внутрішня · від активного знімка вниз.
