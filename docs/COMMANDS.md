...
| `./bdo tui` | вікно-монітор у терміналі: стан, журнал, прогін без браузера |
...
| `./bdo session [new&#124;close&#124;list&#124;show&#124;journals&#124;delete]` | сесія роботи: історія пачок і повні дані до видалення закритої сесії |
| `./bdo web [--background&#124;--status&#124;--stop&#124;--no-open]` | ОСНОВНИЙ інтерфейс (те саме, що ./bdo без аргументів): сторінка в браузері, вільний порт, посилання в терміналі |
| `./bdo desktop (--install&#124;--uninstall&#124;--status)` | Linux-ярлик у меню програм для запуску браузерного інтерфейсу |
...
| `./bdo timing [N&#124;--all]` | куди пішов час прогону: модель проти решти кроків · **для розробки**, у флоу перекладу не викликається |
| `./bdo bench <модель> [<модель>…]` | порівняти моделі на тих самих payload і промптах; без запису в API · **для розробки**, у флоу перекладу не викликається |
...
| `./bdo models list&#124;select&#124;clear&#124;load&#124;unload&#124;probe&#124;lock&#124;unlock&#124;settings` | перелік моделей Ollama/oMLX, здатності роздумів, поведінкова проба й операційний вибір |
