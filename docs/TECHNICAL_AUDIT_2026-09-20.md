...
### A-09. Повний gate дублював селективну перевірку — закрито

Дослідження показало, що повна відмова від перевірок знизила б якість, а
сторонній test runner не вирішив би проєктно-специфічне зіставлення шляхів із
інтеграційними тестами. Водночас `gate full` не був необхідним: локальний
`gate touched` розростався через всі коміти від `origin/main`, а невідомі
шляхи покладався перевіряти повний CI suite.

Контракт замінено на fail-closed селекцію: локально перевіряється робочий diff,
CI передає точну базу push/PR, кожен відомий шлях має mapping, а невідомий
шлях зупиняє gate. Механізм карти й видимість пропусків мають окремі короткі
regression-тести. `gate full` видалено, а якість тримається не кількістю
запущених тестів, а доказовим зв’язком між зміною та перевіркою.
