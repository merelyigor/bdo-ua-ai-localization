...
- **«жодного `.sh` у шляху виконання»** · виконано на Windows (`bdo.bat` →
  `php cli\bdo.php`), але НЕ на Unix: `./bdo` лишається bash і йде через
  диспетчер-`.sh` (45 із 70 файлів `cli/**`). Це СВІДОМО · заморожене тіло під
  диспетчером є шляхом відкату, і прибирати його зараз означає втратити його;
