...
**ЕТАП ЗАКРИТО 2026-09-18 · підсумок і числа в §10.** Нижче лишається хід
роботи; чинним станом є §10.

**Стан на 2026-09-14.** Підетап 9.2 виконано: старий rollback-шлях
...
- **Статус:** закрито 2026-09-18 · див. §10
- **Створено:** 2026-09-07
...
- **Мету скориговано власником 2026-09-12** · див. §1: PHP є ЗАСОБОМ, а не
  ціллю етапу.

## 1. Мета

Кінцева мета · перекладати патчі й хоч цілу гру, тримаючи GUI запущеним 24/7,
поки власнику це потрібно. Набір мусить запускатись як ЗВИЧАЙНА ПРОГРАМА, а не
як бінарник у консолі:

- Windows · `.bat`, macOS · окремий `.app`, Linux · свій еквівалент;
- запуск працює у ФОНІ й піднімає локальний сервер;
- сервер дає БРАУЗЕРНИЙ GUI, у якому й іде вся робота з перекладами через
  локальні LLM і не тільки;
- прогін живе автономно стільки, скільки треба власнику, без нагляду за
  терміналом.

Звідси й вимога до PHP: одна мова на шляху виконання потрібна, ЩОБ такий
запуск був однаковим на macOS, Linux і Windows БЕЗ WSL2, без Docker і без
зовнішніх утиліт, крім PHP CLI. Прибирання bash зі шва між командою й логікою
є засобом досягти цього, а не самоціллю.

Це НЕ переписування проєкту: логіка вже в PHP.

Наслідок для пріоритетів: `cli/system/web.sh` → `WebCommand` і точки входу
(§7.5, §8) не є «залишковими файлами» · вони і є ядро продукту, бо саме вони
піднімають те вікно, у якому власник працює.
...
**IN SCOPE (уточнено 2026-09-12 за §18 нормативу):** лише ПРОДУКТ · те, що
бере участь у перекладі рядків. `bdo`, `bdo.bat`, `bin/tui.sh`, сторінка й
сервер, `cli/**` у шляху прогону, `docs/COMMANDS.md`.

**ПОЗА SCOPE назавжди · ОСНАСТКА РОЗРОБКИ.** Лишається на bash і боргом не є:
`scripts/agent-check.sh` (1 285 рядків), `tests/**` (68 файлів, 12 577 рядків),
git-хуки, `cli/audit/**` (8 файлів, 1 008 рядків), `./bdo runtime`, `inspect`,
`browser`, `gif`, `mac-app`. Межа проводиться фактом: команда належить
продукту, якщо її кличе сторінка (`lib/Run/Actions.php`), драйвер прогону або
`flow_commands` реєстру. Зміряно 2026-09-12: сторінка тисне `env`, `loop`,
`mode`, `run`, `session`, `watch`, `moderation`, `patch` · жодної команди з
`cli/audit/**`.

**УТОЧНЕННЯ ВЛАСНИКА 2026-09-17 · межу зсунуто.** Дослівно: «sh можна
використати тільки для розробки локально, але не для роботи самого проекту ·
GUI та флоу перекладів тощо». Це ширше за межу 2026-09-12 у двох місцях, і
обидва зміряні, а не припущені (§9):

1. Критерій «кличе сторінка» був неповним · він не рахував ДРУГУ поверхню
   продукту, вікно `./bdo tui`. Воно кличе `./bdo review`, тобто
   `cli/audit/project-review.sh` (`bin/tui.sh:162`), тому цей файл належить
   продукту, а не оснастці.
2. Операторські звіти над даними конвеєра (`judge`, `quarantine`, `incidents`,
   `suspects`, `timing`, `bench`, `models-run`, `runtime`, `platform`) є
   «роботою самого проекту»: `./bdo quarantine --clear` змінює вибірку
   наступних пачок, і всі вони дозволені guard-у сторінки в
   `cli/command-registry.json`. На Windows, заради якого існує етап, жоден із
   них не працює взагалі.

Оснасткою РОЗРОБКИ лишаються: `tests/**`, `scripts/**` (гейт, збірка значка,
делегування), `cli/system/browser-check.sh`, `ide-inspect.sh`, `tui-gif.sh`,
git-хуки. Плюс два файли, які лишаються bash за технічною необхідністю, а не
за межею scope: `gui-path.sh` і `mac-app.sh` (§9).

**Старий IN SCOPE (до уточнення):** `cli/**` усі `.sh`, тести під перенесені
частини.
...
змінився на `86cd48e`. Причина закрита в контракті промптів (теку `docs/ai-workflow/` знято 2026-09-12, історія · тег `ai-workflow-final`).
...
- **3b · глосарій і терміни (617 рядків):** `glossary-list.sh` (126, курсорна
  ...
  **Борг, зафіксований на 3c.** Заголовок-диспетчер у
  ...
  того, щоб вирішити, чи підвантажувати старий shell-resolver середовища. Заразом зʼявився новий
...
- **3c · серце конвеєра (436 рядків):** **Статус:** прийнято · рев'ю
  ...
  **Наслідок для підетапу 7.** Одне джерело правил вибору цілі зробити НЕ
  вдалось, і причина законна: PHP мусив би запускати старий shell-resolver
  ...
  `ApiEnvironment.php` дублює правила вибору цілі зі старим shell-resolver,
...
**Статус:** у роботі · пілот 7.1 закритий (`session-timer`). Рішення власника
2026-09-12: з цього підетапу етап веде ОДНА локальна сесія, флоу
АРХІТЕКТОР+ВИКОНАВЕЦЬ знято (`AGENTS.md`, «Хто ти в цьому проєкті»). Пакети,
review-кола й HANDOFF не застосовуються; приймання · `./bdo gate full` із
кодом 0, CI на точному SHA і перевірка власником. Частини роботи йдуть у
Codex за `docs/DELEGATION.md`; рішення лишаються в головній сесії.

**Порядок переносу, встановлений інвентаризацією 2026-09-12.** Він визначений
не розміром файлів, а СПОСОБОМ ВИКЛИКУ: файл, який інші скрипти `source`-ять,
експортує змінні в їхнє оточення, і перенос у PHP розриває цей зв'язок, поки
bash-викликачі живі.

| Черга | Файли | Чому саме тут |
|---|---|---|
...
| 7.2 · СКАСОВАНО | `browser-check.sh`, `ide-inspect.sh`, `tui-gif.sh`, `check-platform.sh` | ОСНАСТКА за §4 і §18: жоден із них не кличе ні сторінка, ні драйвер · лише `bdo`. Черга складалась до уточнення scope 2026-09-12 і суперечила йому. Лишаються на bash |
...
| 7.5 | `watch.sh`, `web.sh` | потребують `lib/Process/Runner.php` (tmux на Unix, відвʼязаний процес на Windows) · найризикованіше, останнім. `mac-app.sh` ВИЛУЧЕНО з черги: за §4 це оснастка, сторінка й драйвер його не кличуть |
| НЕ ЗАРАЗ | старий env-resolver, старий paths-resolver | їх source-ять 20+ і 8 bash-скриптів; переносяться аж у підетапі 8, коли bash-викликачів не лишиться |
| НІКОЛИ | `gui-path.sh` | сорситься в `bdo:20` ЩОБ ЗНАЙТИ php (клас D89). Перенести в PHP означає вимагати php до того, як його шлях полагоджено |
| ВИДАЛИТИ · зміряно 2026-09-17 | `timed.sh` | рядок «НІКОЛИ» нижче втратив підставу разом із переносом драйвера: `RunDriveCommand::timedCapture()` і `RunLoopCommand::timedProcess()` пишуть мітки В ПРОЦЕСІ, і обгортку не кличе ніхто · живих згадок нуль (`lib/Cli/Command/Run/RunLoopCommand.php:553` · коментар). Файл мертвий, а `tests/step-times.sh` перевіряє мертвий код: властивість D81 тепер мусить триматись на PHP-мітках, інакше вона не тримається ніде |
| НІКОЛИ (підстава знята) | `timed.sh` | обгортка ВИМІРЮВАННЯ навколо чужих команд, і її робота · пережити зламаний php. `tests/step-times.sh:98-106` тримає саме це: колись памʼять зайняла модель на 23 ГБ, php не піднявся, обгортка вбила крок, і драйвер сказав «роль не дала відповіді», хоча роль ВІДПОВІЛА. Диспетчер у PHP інвертує її призначення: спроба переносу 2026-09-13 одразу завалила цей тест кодом виходу 1 замість 0 і була відкочена |

Мертвий шим check-rules видалено 2026-09-12, а не перенесено: жодного викликача в
репозиторії, і сам файл був зламаний · рахував `ROOT` як власну теку й кликав
неіснуючий шлях cli/system/scripts/agent-check.sh.

**Уточнення 2026-09-13 · `timed.sh` вилучено із залишку.** Пакет 7.4 брав його разом із `session.sh`, і це була помилка головної сесії, а не виконавця: обгортка вимірювання мусить працювати ТОДІ, КОЛИ php не працює, тому переносу не підлягає взагалі (рядок «НІКОЛИ» вище). Залишок продукту після уточнення: `session.sh` (перенесено цим пакетом), `watch.sh`, `web.sh` і `run-stop.sh` · разом 907 рядків.

**7.5 закрито 2026-09-13 (версія 6.8.4).** `watch.sh` (112) і `web.sh` (419)
перенесено в `WatchCommand`/`WebCommand`, заморожені тіла лишились без жодного
вилучення. Перенос коштував ТРЬОХ виправлень, і всі три · про ЖИВИЙ процес, а
не про логіку: (1) stdio сервера не відвʼязані від викликача; (2) сервер
успадковував PIPE на fd 5 і 6 від сусідніх `proc_open`, тому `$(...)` ніколи не
бачив EOF · закрито оболонкою, що закриває всі числові дескриптори перед
`exec`; (3) при зайнятому типовому порту перевірка здоровʼя йшла не на той
порт, на якому слухав сервер · зависання замість фолбеку. Урок для §8: у PHP
дескриптори успадковуються за замовчуванням, і для фонового застосунку це
основний клас дефектів. `lib/Process/Runner.php` не знадобився. Залишок
продукту · `run-stop.sh` (43 рядки).
...
`php.exe`. Доказ не текстовий: job `Windows native PHP smoke` запускає САМЕ
`.bat` у теці з пробілом і кирилицею, на PATH без `bash.exe`, і звіряє факти ·
`bdo.bat native web: status=0; second-same-url=1; stop=0; port-free=1;
printed-url=1`. Два кола коштували не продукт, а перевірка: спершу вона падала
без причини (мовчазне повідомлення · той самий клас, що й на Linux), потім
виявилось, що `ArgumentList` .NET екранує лапки не за правилами `cmd`. Окремо
відкочено зміну, яку виконавець зробив із хибного доказу: `:0` у гілці
зайнятого порту працює, а «Operation not permitted» він побачив у власній
пісочниці, яка не дає bind. Попередній вибір вільного порту сокетом має щілину
між вибором і прив'язкою, `:0` її не має за побудовою.

**Підетап 8.2 ЗАКРИТО 2026-09-13 (версія 6.9.3).** Linux дістав запуск
ярликом: `./bdo desktop --install` кладе `.desktop` у
`${XDG_DATA_HOME:-~/.local/share}/applications/` з АБСОЛЮТНИМ `Exec`,
`Terminal=false` і значком `bdo.png` (те саме обличчя, що в `BDO.app`).
Ярлик не можна покласти в репозиторій готовим · тека набору в кожного своя,
тому шлях підставляється при встановленні.

Доказ не текстовий: `tests/linux-desktop.sh` ВИТЯГУЄ рядок `Exec` із
створеного файла і ВИКОНУЄ його, після чого звіряє підняте посилання,
повторний запуск без другого сервера і звільнений порт. На runner додатково
працює власний валідатор формату: `desktop-file-validate: код 0`. `grep` по
тексту ярлика доказом не вважався жодного разу.

Гейт тут спрацював як і мав: новий тест приніс із собою конвеєри
`printf … | grep -q`, і `check_sigpipe_pipelines` завалив прогін за секунди
(клас D114). Переписано на herestring, повторний гейт зелений.

**Підетап 8.3 ЗАКРИТО 2026-09-14 (версія 6.9.8).** Вхід на Unix більше не
bash: `bdo` має шебанг `#!/usr/bin/env php`, а маршрут (66 гілок `sh_run`,
7 `php_or_sh`, вкладені групи) переїхав у `lib/Cli/Router.php`. Оснастку
маршрутизатор запускає через `pcntl_exec`, а не `proc_open` · заміна процесу
не чіпає ні дескрипторів, ні кодів виходу, тому `tui`, `gate` і `watch --show`
лишаються в живому терміналі. Лагодження PATH для клікового запуску (D89)
переїхало вгору по ланцюгу · у `cli/system/mac-app.sh` і `Makefile`, бо
PHP-вхід не може знайти php до того, як php запустився.

Доказ · ПОБАЙТОВИЙ еталон старого входу, знятий ДО змін (`a34b3d7`, git
worktree): 53 кореневі команди й 31 вкладена, обидва режими (`BDO_SHOW_HELP`
і звичайний), однакова синтетична ціль на мертвому порту з обох боків.
Розходжень немає, крім двох УЗГОДЖЕНИХ: `mode start` і `mode status` без
аргументів раніше давали bash-шум із номером рядка (`./bdo: line 186: 3: ...`),
тепер · названу причину.

Еталон окупився двічі. Перше: маршрут `context` називав PHP-команду
`row-context` (імʼя ФАЙЛА, а не команди), тому `./bdo context <hash>`
відповідав «невідома команда». Клас закрито `Kernel::knowsCommand()` ·
кожна названа маршрутом команда мусить існувати в ядрі. Друге: `BDO_SHOW_HELP`
мовчки перестав діяти для частини вкладених маршрутів, і `./bdo help terms
submit` замість довідки ЗАПУСКАВ команду, яка пише пропозиції.

Шість перевірок стерегли дослівні bash-рядки старого входу й були
переадресовані на таблицю маршрутизатора, кожна з доказом падіння:
`tests/command-registry.sh`, `mechanical-before-qa.sh`, `registry-hygiene.sh`
(два місця), `cli-kernel.sh` і `patch-argument.sh`. Остання стала
ПОВЕДІНКОВОЮ: `./bdo mode start patch 20 7` на мертвому порту й у тимчасовому
стані · номер патча видно у фільтрі, який пишеться до спроби вибірки (падає за
0 с, якщо аргумент загубити).

Окремо чесно: у `tests/cli-run-mode-parity.sh` оголошення цілі виведене з
побайтового порівняння, бо заморожений bash друкує його ДВІЧІ, а PHP · один
раз. Послаблення закінчується на кількості; спроба додати звірку ТЕКСТУ цілі
була відкинута як фіктивна · саботаж довів, що рядок на обох шляхах друкує той
самий env-resolver і розійтися не може.

## 8а. Стан на 2026-09-13 · що доведено, а що ні

Підетапи 1-8 у стані `реалізовано`. `прийнято` ставить власник · за реєстром
статусів це не право агента.

Доведено механічно:

- `./bdo gate full` зелений на macOS (локально) і на ubuntu-24.04 (CI) ·
  точний SHA `1a0caaf`, усі чотири job зелені;
- запуск-як-програма є на всіх трьох ОС: `bdo.bat` (нативний PHP, доведено
  job `Windows native PHP smoke` на PATH без `bash.exe`), `BDO.app`
  (`tests/mac-app-quit.sh`), `.desktop` (`tests/linux-desktop.sh`);
- сам `bdo` не потребує ні `jq`, ні `curl`, ні `tmux` · вони лишились тільки
  в коментарях.

Чого НЕ доведено, і це не можна ховати:

- **«перевірено власником»** · не виконано. Windows перевірив CI, а не
  людина на своїй машині;
- **«жодного `.sh` у шляху виконання»** · було заявлено як виконане 2026-09-13
  і це ПЕРЕБІЛЬШЕННЯ, зняте інвентаризацією 2026-09-17 (§9). Виконано для
  СТОРІНКИ й ДРАЙВЕРА: `bdo` має шебанг `#!/usr/bin/env php`, маршрут живе в
  `lib/Cli/Router.php`, старі dispatcher-тіла `.sh` видалено, Windows ·
  `bdo.bat` → `php cli\bdo.php`. НЕ виконано для другої поверхні продукту:
  `./bdo tui` і досі є `bin/tui.sh` (396 рядків bash), і він тягне за собою
  `cli/audit/project-review.sh`. Формулювання трималось на тому, що «шлях
  виконання» мовчки означало сторінку й драйвер;
- **живий прогін на 50 рядках новим шляхом** · не виконано. Причина не
  технічна: ціль у `.env` зараз `PROD`, тобто прогін пише в бойові дані, і
  рішення про його запуск належить власнику, а не агенту.

**Залишок продукту зміряно 2026-09-12 (уточнення scope).** За критерієм §4
(кличе сторінка, драйвер або `flow_commands`) у підетапі 7 лишаються рівно
чотири файли: `timed.sh` (69 рядків, драйвер кличе 4 рази), `session.sh` (333,
сторінка 19), `watch.sh` (112, сторінка 9 і драйвер 1), `web.sh` (419,
сторінка 5). Плюс `run-stop.sh` (43) з `cli/run/`. Разом 976 рядків · стільки
ж, скільки мала порція 4c, яка пройшла одним колом. Решта теки · оснастка й
переносу не підлягає.
...
### Підетап 9 · тести · СКАСОВАНО 2026-09-12

**Статус:** скасовано рішенням власника. Причина в §18 нормативу: тести й гейт
не беруть участі в перекладі рядків, на Windows власнику не потрібні, а
переписування 12 577 рядків дало б нуль користі продукту. Додатковий аргумент
проти переносу: тести на іншій мові не успадковують дефектів того, що
перевіряють · саме тому вони й ловлять розходження між оркестраторами.

Історичне формулювання лишається нижче як запис того, що планувалось.

**Мета (скасована).** `tests/**` (7 895 рядків bash) · на PHP, щоб gate працював на
Windows.
...
## 9. Інвентаризація shell 2026-09-17 · що лишилось і чим закривається

Знято командою `git ls-files '*.sh'` на `2a30628`: **95 файлів, 18 150 рядків**.
Розподіл і вирок по кожній групі нижче. Вирок стоїть не за текою, а за
відповіддю на одне питання: чи бере файл участь у роботі продукту (§4,
уточнення власника 2026-09-17).

| Тека | Файлів | Рядків | Вирок |
|---|---:|---:|---|
| `tests/**` | 73 | 13 838 | лишається bash · підетап 9 скасовано 2026-09-12 |
| `scripts/**` | 4 | 2 056 | лишається bash · гейт, збірка значка, делегування |
| `cli/audit/**` | 8 | 1 007 | 1 продукту, 6 операторських, 1 уже переноситься |
| `cli/system/**` | 7 | 678 | 2 лишаються назавжди, 1 мертвий, 4 оснастка або оператор |
| `cli/runtime/**` | 2 | 175 | обидва в чергу 9.4 |
| `bin/tui.sh` | 1 | 396 | ПРОДУКТ · головний залишок етапу |

### 9.1 Залишок ПРОДУКТУ · черга 9.3

| Файл | Ряд. | Хто кличе · доказ | Робота |
|---|---:|---|---|
| `bin/tui.sh` | 396 | `Router.php:23` (`./bdo tui`), guard `^\./bdo tui$` у реєстрі | перенести в `Cli\Command\System\TuiCommand`. PTY-частина (`clear`, кольори, `read`) іде через `posix_isatty`, як і планувалось у §5.2; аргументи прогону вікно вже не складає само · бере план у `cli/run/plan-args.php`, і ця межа не рухається |
| `cli/audit/project-review.sh` | 121 | `bin/tui.sh:162` · екран «стан», плюс `Router.php:65` | переноситься РАЗОМ із вікном або раніше: доки він bash, `./bdo tui` на Windows лишається неповним навіть після переносу самого вікна. Перенос почала паралельна сесія 2026-09-17 (§9.2) |
| `cli/runtime/env-sync.sh` | 9 | `Router.php:31` (`./bdo sync`) | три рядки обгортки навколо `cli/runtime/env-sync.php` і `./bdo env`; переноситься в `EnvSyncCommand` без жодного рішення |
| `cli/system/timed.sh` | 69 | НІХТО · живих викликачів нуль | ВИДАЛИТИ, а не переносити. Разом із ним переадресувати `tests/step-times.sh` на PHP-мітки (`RunDriveCommand::timedCapture`, `RunLoopCommand::timedProcess`), інакше властивість D81 лишиться без перевірки взагалі |

### 9.2 Операторські команди над даними конвеєра · черга 9.4

Вони не «розробка локально»: `./bdo quarantine --clear` змінює вибірку
наступних пачок, решта читає журнали прогону. Усі дозволені guard-у сторінки,
і жоден не працює на Windows.

`quarantine-report.sh` (121), `judge-report.sh` (84), `model-incidents.sh` (91),
`model-run.sh` (120), `model-bench.sh` (244), `glossary-suspects.sh` (160),
`timing-report.sh` (66), `check-runtime.sh` (166), `check-platform.sh` (215).
Разом **1 267 рядків**.

**Пʼять із них уже в роботі паралельної сесії** станом на 23:15 2026-09-17:
`lib/Cli/Router.php` у робочій теці (незакомічено) перемикає `review`,
`timing`, `incidents`, `quarantine` і `judge` з `kind => script` на
`kind => php`, і поруч зʼявився `lib/Cli/Command/Audit/TimingReportCommand.php`.
Ця черга описує, що лишається ПІСЛЯ них: `model-run.sh`, `model-bench.sh`,
`glossary-suspects.sh`, `check-runtime.sh`, `check-platform.sh` · 905 рядків.

Порядок усередині черги · за ризиком: спершу ті, що лише ЧИТАЮТЬ
(`timing`, `judge --list`, `incidents`, `quarantine --list`, `review`), останнім
`quarantine --clear`, бо він єдиний ПИШЕ в `state/**` і обнуляє
`row-attempts.jsonl`.

### 9.3 Лишається bash назавжди · з причиною, а не за замовчуванням

| Файл | Ряд. | Чому саме він |
|---|---:|---|
| `cli/system/gui-path.sh` | 84 | сорситься ДО php, щоб його знайти (D89, D106). PHP-версія вимагала б php раніше, ніж полагоджено його шлях |
| `cli/system/mac-app.sh` | 23 | те саме для `BDO.app`: LaunchServices дає мінімальний PATH, і виправити його після шебанга вже неможливо. Логіки в файлі немає · `exec php cli/bdo.php mac-app` |
| `scripts/**` | 2 056 | гейт, збірка значка, делегування · оснастка розробки |
| `tests/**` | 13 838 | доказ переносу не має права успадкувати дефекти того, що перевіряє (підетап 9, скасовано) |
| `cli/system/browser-check.sh`, `ide-inspect.sh`, `tui-gif.sh` | 287 | Chrome для агента, інспекції IDE, GIF для документації · власнику не потрібні й на Windows не потрібні |

### 9.4 Означення готовності, зведене до перевірюваного

Етап закритий, коли все нижче дає названий результат, а не оцінку:

1. `rg -l '\.sh' lib/Cli/Router.php` не містить жодного маршруту продукту ·
   лишаються тільки рядки з §9.3;
2. `./bdo tui` піднімається без `bash` у PATH (та сама перевірка, що вже
   ганяє `run-loop` у job `Windows native PHP smoke`);
3. `tests/tui-live.sh` і `tests/tui.sh` проходять на новій реалізації БЕЗ
   послаблення сценаріїв · вони і є доказ, що вікно не змінило поведінки;
4. `cli/system/timed.sh` видалено, а мітки часу має чим перевірити;
5. `./bdo gate full` зелений локально і в CI на точному SHA.

## 10. ЕТАП ЗАКРИТО · 2026-09-18

Перехід завершено повністю. Означення готовності (§8) виконано в частині, що
стосується bash: **жодного `.sh` у роботі програми**.

Знято тим самим способом, що й інвентаризація §9 — `git ls-files '*.sh'`:

| Де | До (2026-09-17) | Після (2026-09-18) |
|---|---|---|
| Робота програми (поза `tests/`, `scripts/`) | 24 файли, 2 807 рядків | **0** |
| Маршрути `kind => 'script'` у `Router` | 8 | **1** · лише `./bdo gate` |
| PHP, що запускає `.sh` | — | **0** (`rg` по `proc_open\|exec\|shell_exec\|system\|popen`) |
| Розробка (`tests/`, `scripts/`) | 14 186 + 2 297 рядків | лишається shell за рішенням власника |

Що саме переписано в цей день: вісім звітів `cli/audit/**` (1 007 рядків),
`check-platform` (215), `check-runtime` (166), `browser-check` (105),
`bin/tui.sh` (398), `tui-gif` (93), `ide-inspect` (89), `gui-path` (84),
`timed` (69), `mac-app` (23), `env-sync` (9). Мертвих `cli/api/*.sh` видалено
551 рядок — їхні PHP-порти існували з версії 7.0.8, а скрипти лежали
недосяжними.

Кліковий вхід більше не тримається на shell: `BDO.app` кличе
`php cli/bdo.php mac-app` напряму, а PATH задано одним рядком у самому бандлі
(`property bdoPath`). `Makefile` перестав source-ити helper. Обидві властивості
стереже `tests/gui-path.sh`, який бере PATH прямо з бандла й запускає ним php.

**Межа тримається кодом, а не памʼяттю.** `./bdo gate shell` має дві стелі, і
обидві стоять на нулі: shell-маршрутів у `Router` і `.sh`-файлів поза
`tests/`/`scripts/`. Кожна може лише падати — гейт не дасть ні додати новий
скрипт, ні лишити стелю завищеною. Окремо перевіряється, що PHP не запускає
`.sh` прихованим `proc_open`.

**Спільний шов для зовнішніх програм · `lib/System/Program.php`.** Він зʼявився
не з архітектурних міркувань: перші три порти принесли оболонку НАЗАД усередину
PHP (`shell_exec("command -v …")`, `shell_exec('curl …')`, `system($cmd)`).
Формально `.sh` не лишалось, фактично `/bin/sh` запускався, і залежність від
платформи нікуди не зникала. Тепер місце знання про зовнішні програми одне:
argv-масив без оболонки, `command -v` замінено обходом `PATH` самим PHP.

### Що знайшлось під час порту й чому це варто прочитати

Порт «один-в-один» дав шість вад, і кожна має спільну природу — перенесена
конструкція означає в PHP інше, ніж у shell:

1. **Регулярка з `sed` у PCRE.** `/?t=` у BRE є літералом, у PCRE `?` є
   квантифікатором. `./bdo browser` через це мовчки казав «інтерфейс не
   працює» при піднятому інтерфейсі.
2. **`substr` по байтах.** `substr($line, 6)` різало префікс «Ціль: » (6
   символів, 10 байтів), і вікно показувало «ь: PROD».
3. **`array_map('json_decode', $рядки, true)`.** Третій аргумент `array_map` є
   ДРУГИМ МАСИВОМ, а не прапорцем асоціативності · екран журналу падав.
4. **`dirname(__DIR__, 5)` замість `4`.** Вікно шукало вхід на рівень вище
   кореня й не знаходило цілі.
5. **`PHP_BINARY` перед входом.** Вікно перестало працювати з будь-яким іншим
   входом, зокрема з підробленим у пісочниці тесту.
6. **`«$argument»` в інтерполяції.** PHP зʼїдає багатобайтовий `»` у назву
   змінної · те саме правило вже стоїть у гейті для bash, і воно потрібне для
   PHP так само.

Приймання ловило їх одним способом: вивід shell-версії знімався ДО правки й
порівнювався БАЙТ У БАЙТ. Без цього всі шість пройшли б як «зроблено».

### Чого етап НЕ закриває

- `./bdo` на Windows без WSL2 власником не перевірявся · це лишається в §8;
- живий прогін на 50 рядках новим шляхом пройдено 2026-09-17 (пачка
  `20260917_043356`, PASS 33 / REVIEW 17 / REJECT 0), але вже після цього дня
  переписано вікно й діагностику · повторний прогін не робився.
