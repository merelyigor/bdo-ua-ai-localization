...
| НІКОЛИ | `timed.sh` | обгортка ВИМІРЮВАННЯ навколо чужих команд, і її робота · пережити зламаний php. `tests/step-times.sh:98-106` тримає саме це: колись памʼять зайняла модель на 23 ГБ, php не піднявся, обгортка вбила крок, і драйвер сказав «роль не дала відповіді», хоча роль ВІДПОВІЛА. Диспетчер у PHP інвертує її призначення: спроба переносу 2026-09-13 одразу завалила цей тест кодом виходу 1 замість 0 і була відкочена |
