...
#   2. `fix` від QA, пропущений фільтром qa-fixes.sh (дрібна правка, >=85%
...
"$SCRIPT_DIR/batch-assert.sh" "$ROWS_FILE" "$CAND_FILE" >/dev/null

QA_FIXES="$BATCH_DIR/heal-qa-fixes.json"
"$SCRIPT_DIR/qa-fixes.sh" "$VERDICT_FILE" "$ROWS_FILE" "$CAND_FILE" > "$QA_FIXES" 2>/dev/null || true
...
if ($forRepair !== []) {
    ...
    printf("\nВИРОК: віддай %s агенту translation-repair, потім КОНТРОЛЬНИЙ QA лише по цих %d рядках - і одразу batch-commit.sh.\n",
        basename($repairFile), count($forRepair));
    echo "Третього кола не буде: те, що лишиться не-PASS, іде в модерацію.\n";
} elseif ($hopeless !== []) {
    echo "\nВИРОК: коло лікування вичерпано. batch-commit.sh: PASS у ШІ-шар, решта в модерацію.\n";
} else {
    echo "\nВИРОК: дефектів не лишилось. Злитий кандидат готовий до batch-commit.sh.\n";
}
...
if [ -s "$REPAIR_PAYLOAD" ]; then
    ...
    if [ -n "$HASHES" ]; then
        ...
        # subset-rows.sh лишає формат API (`data.rows`), якого чекає RowSet;
        # сам payload плаский, і build-schema.sh його не прочитає.
        ...
        if "$SCRIPT_DIR/subset-rows.sh" "$ROWS_FILE" "$HASHES" "$SUBSET" >/dev/null 2>&1 \
           && "$SCRIPT_DIR/build-schema.sh" "$SUBSET" >/dev/null 2>&1 \
           && "$SCRIPT_DIR/build-schema.sh" --qa "$SUBSET" >/dev/null 2>&1; then
