...
final class Row
{
    ...
    /**
     * Глосарій рядка, розділений за ПОХОДЖЕННЯМ відповідника.
     *
     * Навіщо розділяти. Заміряно 2026-09-04: із 136 022 записів каталогу
     * 131 391 має `ukrainian_layer: machine`, а `manual` · 172. Тобто
     * «затверджений відповідник», який промпт називає законом, у 96,6%
     * випадків є машинною здогадкою такої самої моделі. Наслідок був видний у
     * вердиктах QA того ж дня: `REVIEW/minor · Недотримання glossary для
     * 'Timing' та 'Move'` · рядок знижено за невживання МАШИННОЇ назви, і в
     * тому ж реченні модель сама визнала candidate допустимим.
     *
     * Але «машинне» не означає «ігноруй»: саме ці 131 тисяча записів є чинним
     * стандартом корпусу, і без них та сама назва отримує пʼять варіантів у
     * межах одного патча (Cheongsa Island · 11 рядків, 3 варіанти). Тому
     * payload віддає ДВА блоки, і промпт трактує їх різно: людське · закон,
     * машинне · чинний стандарт, чия відсутність дефектом не є.
     *
     * Внутрішня механіка (`glossaryCaseViolations`, `fixGlossaryCase`) далі
     * працює з повним переліком: виправлення регістру детерміноване й
     * походження не залежить.
     *
     * @return array{human: array<string,string>, machine: array<string,string>}
     */
    public function glossaryByLayer(): array
    {
        $layers = $this->glossaryLayers();
        $split = ['human' => [], 'machine' => []];
        foreach ($this->glossary() as $source => $ukrainian) {
            $key = ($layers[$source] ?? '') === 'machine' ? 'machine' : 'human';
            $split[$key][$source] = $ukrainian;
        }
