 ...
 *
 * Тека пачки є робочим станом під час прогону й доказом сесії після нього.
 * Вона живе до явного видалення закритої сесії, до якої пачку привʼязано.
 */
final class Workspace
{
    ...
    {
        // У НАЗВІ ТЕКИ · КОРОТКИЙ ХВІСТ, У МАНІФЕСТІ · ПОВНИЙ КЛЮЧ.
        //
        // 16 символів відбитка робили ідентифікатор довжиною 32 символи, і на
        // екрані сесій він з'їдав колонку, нічого не додаючи оку (власник
        // 2026-09-19). Хвіст тут потрібен лише щоб відрізнити дві пачки, узяті
        // в одну секунду, і щоб упізнати набір поглядом · для цього шести
        // символів досить. Перевірка «чи це рядки цієї пачки» на нього НЕ
        // спирається: `assertRows()` звіряє повний `identity_key` з маніфеста,
        // і він лишився незмінним.
        $base = $stamp.'_'.substr($rows->key(), 0, 6);
        $workspace = null;
        ...
        $workspace->writeManifest([
            ...
            'artifacts' => [],
            'write_receipt' => null,
            // ДОЗВІЛ НА ЗАПИС ЖИВЕ В ПАЧЦІ, А НЕ В ОТОЧЕННІ ПРОЦЕСУ. Раніше
            // драйвер питав `BDO_DRY_RUN` у себе, і запис був дефолтом · тому
            // пачка лишалась тестовою рівно доти, доки жив процес, який її
            // запустив. 2026-09-17 продовження пачки `20260917_024429` з іншого
            // процесу записало в PROD 43 рядки без жодного підтвердження.
            // Тепер рішення ухвалюється один раз, у момент народження пачки.
            'write' => getenv('BDO_DRY_RUN') === '1' ? false : (getenv('BDO_WRITE') === '1'),
        ]);
