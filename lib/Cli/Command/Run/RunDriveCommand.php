...
final class RunDriveCommand implements Command, \Bdo\Translate\Cli\CommandHelp
{
    ...
    private function refreshMechanicalVerdicts(): void
    {
        ...
        foreach (is_array($verdicts) ? $verdicts : [] as $verdict) {
            ...
            if ($defects === []) {
                // ДЕФЕКТУ БІЛЬШЕ НЕМА · І ВИРОК МУСИТЬ КАЗАТИ САМЕ ЦЕ.
                //
                // Статус тут раніше лишався `REJECT critical`, і на екрані
                // виходила неправда: червоне «відхилено · critical» на рядку,
                // який ремонт полагодив і який їде в шар. Власник 2026-09-18
                // побачив «відхилено 2» там, де насправді все 50 рядків
                // пройшли, і сказав прямо: напис має відображати дійсність.
                //
                // `REVIEW minor` є правдою: рядок чистий, але ПОВТОРНОЇ
                // перевірки якості не проходив · QA бачив його до ремонту.
                //
                // Зміна безпечна, і це не судження, а властивість коду:
                // `ChannelRouter` рахує механіку на ФІНАЛЬНОМУ тексті й вона
                // сильніша за статус. Ремонт справді полагодив · рядок їде
                // далі; не полагодив · механіка відправляє його до людини за
                // будь-якого статусу (перевірено всіма чотирма комбінаціями).
                $verdict['status'] = 'REVIEW';
                $verdict['severity'] = 'minor';
                $verdict['issue'] = 'полагоджено ремонтом · повторної перевірки якості рядок не проходив';
            } else {
                $verdict['issue'] = $prefix.implode('; ', $defects);
            }
            $refreshed[] = $verdict;
        }
