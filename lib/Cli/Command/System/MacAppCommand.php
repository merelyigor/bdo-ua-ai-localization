...
final class MacAppCommand implements Command
{
    ...
    private function start(Output $output): int
    {
        ...
        if ($url === null) {
            ...
            // СЕРВЕР ПІДНІМАЄТЬСЯ НЕ МИТТЄВО · питати його стан один раз було
            // перегонами, і власник часто бачив «посилання не знайшлося» на
            // цілком справному запуску (скарга 2026-09-18). `--background`
            // повертає керування, щойно віддав процес, а `state/web.json`
            // і відповідь `/api/health` зʼявляються трохи згодом.
            //
            // Тому питаємо кілька разів протягом пʼяти секунд. Це не «поріг
            // терпіння», а рівно те, скільки триває старт: у нормі посилання
            // знаходиться з першої-другої спроби, а довше чекати нема чого ·
            // тоді це вже не повільний старт, а справжня відмова.
            $url = null;
            for ($attempt = 0; $attempt < 25 && $url === null; $attempt++) {
                if ($attempt > 0) {
                    usleep(200000);
                }
                [, $message] = $this->web(['--status']);
            ...
            if ($url === null) {
                // Показуємо ПРИЧИНУ, а не пропозицію показати повідомлення
                // агенту: власник бачить діалог замість терміналу, і без
                // тексту сервера він не може ні зрозуміти, ні переказати, що
                // саме сталося.
                $why = trim($message) === '' ? 'сервер нічого не відповів' : trim($message);
                return $this->dieGui($output, "Інтерфейс піднявся, але посилання на сторінку не знайшлося за 5 секунд.\n\n{$why}\n\nПокажи це повідомлення агенту.");
            }
        }
        $this->open($url);
        $output->stdout($url."\n");
