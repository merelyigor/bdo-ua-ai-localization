 ...
 */
final class WebCommand implements Command, \Bdo\Translate\Cli\CommandHelp
{
    /** @var resource|null */
...
Локальний інтерфейс у браузері: одна команда, вільний порт, посилання в терміналі.
  ...
  ./bdo web                 # запустити й тримати; Ctrl-C зупиняє
  ./bdo web --background    # запустити й відпустити термінал
  ./bdo web --status        # чи працює, на якому порту, чи відповідає
  ./bdo web --stop          # зупинити фоновий сервер
  ./bdo web --no-open       # не відкривати браузер самому
