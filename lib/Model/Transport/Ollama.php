...
final class Ollama implements Transport
{
     ...
     *
     * Тут свідомо НЕМАЄ `keep_alive`. 2026-09-06 я поставив його в запит
     * (30 хвилин), щоб прибрати повторне завантаження ваги між пачками · і це
     * було втручання не у свій конфіг: власник тримає власні налаштування
     * Ollama, у нього модель вивантажується через 5 хвилин, і запит із нашим
     * значенням тихо перекривав його рішення на кожному виклику.
     *
     ...
     * паралелізм і скільки моделей тримати · вирішує машина, а не ми.
     */
    ...
    public function send(Request $request, ?callable $onChunk = null): Reply
    {
        ...
        // ЧОМУ ТУТ РОЗМІР ШМАТКА 1.
        //
        // Ollama віддає по рядку NDJSON на токен, але обгортка `http` читає
        // блоками по 8192 байти й віддає `fgets` лише те, що вже в буфері.
        // Заміряно 2026-09-06 на живій пачці: журнал токенів ріс раз на 1.3 с
        // одразу на ~1 КБ · тобто півсотні токенів приїжджали разом. На екрані
        // це давало друк «блоками по рядку» навіть при бездоганному буфері на
        // сторінці (D85). З розміром шматка 1 байт читання повертається одразу,
        // щойно байт прийшов, і темп показу дорівнює темпу генерації.
        //
        // Ціна · системний виклик на байт. Відповідь ролі це сотні кілобайт,
        // тобто сотні тисяч читань із локального сокета: на тлі десятків
        // секунд самої генерації це непомітно, а показ стає чесним.
        ...
        if ($final === []) {
            ...
            // на роздуми, ні на відповідь (7.7.0), а зациклення ловить частка
            // повторів (7.8.0). Але `timeout` контексту `http` є таймаутом
            // ЧИТАННЯ: коли рантайм не віддав НІ ОДНОГО байта, читання
