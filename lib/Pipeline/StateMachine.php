...
final class StateMachine
{
    ...
    /**
     * Стан, із якого пачка вже нікуди не піде.
     *
     * Береться з ТІЄЇ САМОЇ таблиці переходів · порожній список означає
     * кінцевий стан за побудовою. Другого переліку кінцевих станів тут немає
     * навмисно: він розійшовся б із таблицею на першій же зміні конвеєра.
     *
     * Невідомий ключ кінцевим НЕ вважається: припущення «нікуди не піде» на
     * стані, якого машина не знає, приховало б справжню поломку.
     */
    public static function isTerminal(string $state): bool
    {
        return array_key_exists($state, self::TRANSITIONS) && self::TRANSITIONS[$state] === [];
    }
}
