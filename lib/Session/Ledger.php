    ...
    private function collectBatches(string $dir): array
    {
        ...
        foreach ($this->readJsonl($dir.'/batches.jsonl') as $entry) {
            ...
            $out[] = [
                ...
                // ТЕСТОВИЙ ПРОГІН ВИГЛЯДАВ ЯК ПРОВАЛЕНИЙ. Пачка без дозволу на
                // запис дає «у шар 0», і в переліку вона нічим не відрізнялась
                // від бойової, яка нічого не записала (власник 2026-09-18).
                // `null` лишається `null` навмисно: пачки, створені до появи
                // поля, не мають права називатись тестовими · невідомо ≠ тест.
