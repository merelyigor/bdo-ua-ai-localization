...
final class Labels
{
    ...
    /**
     * Стан пачки, яку вже ніхто не веде · підпис для ІСТОРІЇ, не для прогону.
     *
     * Знайдено 2026-09-18 на екрані сесій: пачка `20260918_043223` показана як
     * «чекає на переклад», хоча її закрито ще вночі, а `20260918_044220`
     * поруч · «закрито». Так само стояла пачка з сесії, закритої два дні
     * тому: «чекає на перевірку якості». Підпис правдивий щодо манифеста й
     * брехливий щодо дійсності · чекати нікому, продовжити цю пачку сторінка
     * не дає взагалі.
     *
     * Тому в ІСТОРІЇ не кінцевий стан читається як обірваний: власник бачить,
     * на якому кроці все спинилось, і не шукає роботу, якої немає. Ключ стану
     * лишається в підказці рядка · нічого не ховаємо.
     *
     * Кінцевий стан («закрито», «зупинено без відновлення») не чіпаємо: він і
     * так каже дійсність.
     */
    public static function stateInHistory(?string $key): string
    {
        $key = trim((string) $key);
        if ($key === '' || StateMachine::isTerminal($key)) {
            return self::state($key);
        }

        return 'облишено на кроці «'.self::state($key).'»';
    }
