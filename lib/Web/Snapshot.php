 ...
 * ПОВЕРХНЕЮ над тим самим прогоном, а не другою системою: вікно (`./bdo tui`)
 ...
 * звіт кроку у `state/run-transcript.log`, і сторінка показує
...
final class Snapshot
{
     ...
     *
     * `state/run-started-at` пише `./bdo run start` при фіксації цілі й знімає при
    ...
    private function batch(array $manifest): array
    {
        ...
        return [
            ...
            'state' => $state,
            // ЧИ МАЛА ПАЧКА ПРАВО ПИСАТИ. Без цього поля тестовий прогін на
            // екрані нічим не відрізняється від бойового, який нічого не
            // записав: в обох «у шар 0» (власник спитав про це 2026-09-18).
            // `null` для пачок, створених до появи поля · невідомо не має
            // права виглядати як «тест».
            'write' => array_key_exists('write', $manifest) ? (bool) $manifest['write'] : null,
    ...
    private function callsView(array $manifest, ?string $sessionId = null): array
    {
        ...
        // НОВА ПАЧКА НЕ ПОКАЗУЄ ЧУЖУ РОБОТУ. Раніше пачка без власних викликів
        // віддавала ВСЮ сесію, покладаючись на підпис «цієї сесії». Підпис не
        // рятує: власник 2026-09-17 запустив новий прогін і побачив під його
        // шапкою шість завершених викликів попередньої пачки · прочитав це як
        // «новий прогін показує старий». Порожній список із названою причиною
        // чесніший за чужі рядки: викликів справді ще не було.
        if ($mine === [] && $all !== []) {
            return [
                'scope' => 'batch',
                'batch' => $batchId,
                'items' => [],
                'reason' => 'ця пачка ще не викликала модель · нижче були б виклики попередньої',
            ];
        }
        if ($mine === []) {
            return ['scope' => 'session', 'batch' => '', 'items' => $all, 'reason' => $reason];
    ...
    /**
     * Хвіст журналу кроків · рівно те, що показує звіт кроку в терміналі.
     *
     * @return list<string>
     */
    ...
    private function stream(bool $full = false): array
    {
        ...
        return [
            ...
            'fresh' => $fresh,
            // ЯКОМУ САМЕ ВИКЛИКУ НАЛЕЖИТЬ ЦЕЙ ТЕКСТ.
            //
            // Сторінка накопичує показане й НЕ дає коротшому хвосту зʼїсти
            // довший (урок D83). Але через це відповідь ПОПЕРЕДНЬОЇ ролі
            // лишалась у вікні, поки нова роль друкувала роздуми: її власний
            // текст ще порожній, а скидати накопичене не було за чим.
            // Власник побачив це 2026-09-19 · у «Відповіді» стояв JSON
            // термінолога, поки перекладач думав. Тепер кожен виклик має
            // власну мітку, і зміна мітки чистить вікна.
            'call_id' => $role === '' ? '' : $role.'|'.$this->streamStartedAt(),
