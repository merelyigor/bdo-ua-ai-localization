...
#
#   source "$SCRIPT_DIR/paths.sh"   # у скрипті
#   ./paths.sh                      # показати, що куди вирішилось
#
...
# діяв би: `select-env.sh` бере з файла лише ключі API, і людина, яка задала
...
# З `.env` беремо ТІЛЬКИ TRANSLATE_*: ключі API - справа select-env.sh, і
...
translate_require_path() {
    ...
    printf 'Задай %s або перевір розкладку: %s/paths.sh\n' "$1" "$TRANSLATE_HOME" >&2
    return 1
}
