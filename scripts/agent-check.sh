...
");
    ...
    # МОДЕЛЬ РОБОТИ ДВОХ АГЕНТІВ. Плани й промпти пише архітектор, код · агент;
    # без цих файлів наступна сесія знову змішає ролі й почне писати код замість
    # промпта (рішення власника 2026-09-07).
    local wf
    for wf in README ROLES WORKFLOW PROMPTS REVIEW HANDOFF; do
        test -f "docs/ai-workflow/$wf.md" \
            || fail "немає docs/ai-workflow/$wf.md · модель роботи агентів неповна"
    done
    # ФАЛЬСИФІКАЦІЯ · головна вимога до виконавця. Без неї промпт отримає
    # фіктивну перевірку: у наборі вже були тест на КОМЕНТАРІ й перевірка, що
    # знаходила `.btn-danger` у `.btn-dangerX`.
    grep -Fq 'ФАЛЬСИФІКАЦІЯ' docs/ai-workflow/PROMPTS.md \
        || fail 'контракт промпта не вимагає фальсифікації перевірок'
    grep -Fq 'фіктивна перевірка' docs/ai-workflow/REVIEW.md \
        || fail 'контракт рецензування не називає фіктивну перевірку блокером'
