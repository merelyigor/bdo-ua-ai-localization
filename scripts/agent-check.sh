...
check_rules() {
    ...
    grep -Fq 'gate touched && ./bdo api' AGENTS.md \
        || fail 'у AGENTS.md немає фінальної перевірки перед завершенням змін'
...
check_delegation_contract() {
    ...
    # справді було, інакше називав би очевидне («усе зробила ця сесія»).
    grep -Fq 'ТІЛЬКИ тоді, коли власник просив' AGENTS.md \
        || fail 'карта правил не робить рядок «ХТО ВИКОНАВ» умовним'
    ...
    grep -Fq 'Мова реалізації · §18' AGENTS.md \
