...
#
# Codex читає `AGENTS.md` робочої теки, а там стоїть правило «спосіб виконання
# питаєш ПЕРЕД роботою». Без цієї преамбули делегований агент слухняно
# перепитує замість роботи, і пакет не виконується взагалі · саме це спіймав
# прогін `delegate-codex` у сусідньому проєкті 2026-09-12. Преамбула йде
# ПЕРЕД промптом і в лог, тому видно, що саме отримала модель.
readonly PREAMBLE='СПОСІБ ВЖЕ ОБРАНО: ти делегований виконавець, власник уже відповів, через що робити задачу. Правило AGENTS.md «спосіб виконання питаєш перед роботою» тебе НЕ стосується · виконуй пакет нижче й нічого не перепитуй. Рішення й розвилки не твої: натрапив на вибір, якого немає в пакеті · зупинись і поверни факти. Не комітити й не пушити.'

set +e
{ printf '%s\n\n' "$PREAMBLE"; cat "$prompt_file"; } \
    | codex exec -m "$MODEL" -s "$SANDBOX" -C "$work_dir" - 2>&1 | tee "$log_file"
status="${PIPESTATUS[1]}"
set -e
