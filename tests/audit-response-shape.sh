...
# (`cli/model/client.php`), а `./bdo audit` вимагав ГОЛИЙ масив.
...
# 9. Правило конверта мусить діяти й у РОБОТІ, а не лише в цьому файлі.
#    Клієнт моделі розпаковує `{"items":[…]}` у масив; якби він цього не робив,
#    конвеєр отримував би обʼєкт замість списку й падав уже на `build-items`.
# Розгортання конверта в роботі перевіряє `tests/schema-provider-compat.sh`
# поведінкою (той самий `cli/model/unwrap.php`, який кличе клієнт), а
# `tests/model-client.sh` · наскрізно. Тут лишається те, чого немає ніде
# більше: що клієнт узагалі кличе цей файл, а не має власної копії правила.
grep -q "unwrap.php" "$ROOT/cli/model/client.php" \
