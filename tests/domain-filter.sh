...
for primary in патч ручний пропозиції покращення-ші; do
    ...
    grep -Fq 'ДОСЛІВНО = скопіюй рядки виводу без змін' "$file" || fail "$primary дозволяє перемальовувати вивід"
    grep -Fq 'РІЗНІ колонки' "$file" || fail "$primary не розрізняє «без ШІ-шару» і стан none"
