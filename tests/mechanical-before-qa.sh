...
# ВИРОК ПІСЛЯ РЕМОНТУ МУСИТЬ КАЗАТИ ДІЙСНІСТЬ (рішення власника 2026-09-18).
#
# Механіку рахують на `clean.json`, тобто ДО ремонту. Далі ремонт може дефект
# прибрати · і тоді червоне «відхилено · critical» на екрані є неправдою:
# власник побачив «відхилено 2» на пачці, де всі 50 рядків пройшли й поїхали в
# шар. Тому `refreshMechanicalVerdicts` перераховує механіку на `healed.json`
# і, коли дефекту більше немає, ставить `REVIEW minor` із прямим текстом.
#
# Це не послаблення: сітку тримає НЕ статус, а механіка на ФІНАЛЬНОМУ тексті
# (`tests/mechanical-final-check.sh`). Ремонт не полагодив · рядок іде до
# людини за будь-якого статусу.
# САБОТАЖ: лишити `REJECT`/`critical` після успішного ремонту · цей блок червоніє.
DRIVE="$ROOT/lib/Cli/Command/Run/RunDriveCommand.php"
grep -Fq "\$verdict['status'] = 'REVIEW';" "$DRIVE" \
    || fail 'полагоджений ремонтом рядок лишається «відхилено» · напис не відповідає дійсності'
grep -Fq 'полагоджено ремонтом' "$DRIVE" \
    || fail 'вирок не називає прямо, що дефект прибрав ремонт'
