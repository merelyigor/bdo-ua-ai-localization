...
// 2в. Недописана транслітерація власної назви · написання береться з ОРИГІНАЛУ.
//
//     Вимір 2026-09-21: у пачці `20260921_114432` шість рядків із двадцяти
//     пішли до людини через одне слово. Модель почала транслітерувати
//     `Illezra` кирилицею й кинула на середині: `Ілlezra`. Заміна латинських
//     двійників кириличними тут робила ГІРШЕ · `l`, `z`, `r` двійників не
//     мають, тому ремонт не міг закрити дефект жодною спробою.
$src = "Illezra has awakened in the Valencia desert.";
$hit = Homoglyphs::find("Ілlezra прокинулась.", $src);
if ($hit === []) $fail("недописана транслітерація не визнана дефектом");
if ($hit[0]["fixed"] !== "Illezra") $fail("написання взято не з оригіналу: ".$hit[0]["fixed"]);
if (! str_contains(Homoglyphs::fix("Ілlezra прокинулась.", $src), "Illezra")) {
    $fail("виправлення знайдено, але не накладається на текст");
}
// Без оригіналу поведінка стара: код не вигадує назву з нічого.
if (Homoglyphs::find("Ілlezra прокинулась.")[0]["fixed"] === "Illezra") {
    $fail("назву відновлено без оригіналу · це вже вигадування");
}
// Відмінена форма точного збігу не дає й лишається ремонту, а не підміні.
$infl = Homoglyphs::find("Сила Ілlezрі зростає.", $src);
if ($infl === [] || $infl[0]["fixed"] === "Illezra") {
    $fail("відмінену форму підмінено називним відмінком з оригіналу");
}
// Два різні токени з тим самим кістяком · вибір неоднозначний, назва не
// вгадується.
$ambiguous = Homoglyphs::find("Ілlezra тут.", "Illezra and ILLEZRA")[0]["fixed"];
if (strcasecmp($ambiguous, "Illezra") === 0) {
    $fail("при двох кандидатах з оригіналу код усе одно обрав один: ".$ambiguous);
}
