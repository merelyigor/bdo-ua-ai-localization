...
TRANSLATE_HOME="$TMP" php "$ROOT/cli/runtime/model-profile.php" env local-fast ollama-local/qwen3.5:9b free >/dev/null
test "$(jq -r '.fingerprint' "$TMP/.opencode/runtime-model-state.json")" != "$fingerprint"
TRANSLATE_HOME="$TMP" php "$ROOT/cli/runtime/model-profile.php" env session-free opencode/x-preview-f-free free >/dev/null
...
for role in translation-terminology translation-worker translation-qa translation-repair translation-judge translation-smoke; do
    test "$(jq -r --arg role "$role" '.agent[$role].model' "$TMP/opencode.json")" = opencode/x-preview-f-free
    grep -Fqx 'model: opencode/x-preview-f-free' "$TMP/.opencode/agents/$role.md"
done
