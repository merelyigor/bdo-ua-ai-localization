...
# --- Налаштування Ollama · чужа територія ------------------------------------
#
# 2026-09-06 я поставив у запит `keep_alive: 30m`, щоб прибрати повторне
# завантаження ваги між пачками. Це було втручання не у свій конфіг: власник
# тримає власні налаштування Ollama (у нього модель живе 5 хвилин), а наш запит
# тихо перекривав його рішення на КОЖНОМУ виклику · без жодного сліду.
#
# Правило: набір надсилає рівно те, що стосується САМОГО ЗАПИТУ · модель,
# промпт, схему, температуру, вікно. Скільки вага живе в памʼяті, який
# паралелізм і скільки моделей тримати · вирішує машина.
for field in keep_alive num_parallel num_gpu main_gpu low_vram numa mmap mlock; do
    grep -Fq "'$field'" "$ROOT/lib/Model/Transport/Ollama.php" \
        && fail "транспорт надсилає налаштування машини «${field}» · воно належить конфігу Ollama власника, а не нам"
    grep -Fq "\"$field\"" "$ROOT/config/roles.json" \
        && fail "у config/roles.json зʼявилось налаштування машини «${field}»"
done
# І прямо: у тілі запиту до Ollama його теж немає.
grep -Fq 'keep_alive' "$SCENARIO_FILE.request" 2>/dev/null \
    && fail 'у запиті до Ollama лежить keep_alive · набір перекриває налаштування машини'


# --- Потік читається БЕЗ буфера ----------------------------------------------
#
# Обгортка `http` у PHP читає блоками по 8192 байти, і `fgets` віддає рядки
# NDJSON злиплими пачками. Заміряно 2026-09-06 на живій пачці: журнал токенів
# ріс раз на 1295 мс одразу на 1066 байтів · півсотні токенів приїжджали
# разом, і жоден буфер на сторінці цього вже не розгладив би (D85). Після
# `stream_set_chunk_size(…, 1)` там же: 55 мс і 48 байтів.
#
# Перевіряється НАЯВНІСТЬ межі в коді транспорту: справжній вимір потребує
# живої моделі, і в gate йому не місце · тому доказ лежить у D85.
for transport in Ollama OpenAi; do
    grep -Fq 'stream_set_chunk_size($handle, 1)' "$ROOT/lib/Model/Transport/$transport.php" \
        || fail "транспорт $transport читає потік буфером · токени приїжджатимуть пачками, і живий друк ітиме блоками"
done
