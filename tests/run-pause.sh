...
# Пошук ПО ЗМІСТУ, а не по рядку. 2026-09-21 цей рядок став багаторядковим
# тернарним виразом (D216 додав окремий стан «зупинено власником»), і перевірка
# почервоніла, хоча сторінка робить рівно те, що треба. Перевірка мусить
# стерегти ПОВЕДІНКУ, а не ширину абзацу.
tr '\n' ' ' < "$ROOT/web/index.html" | tr -s ' ' | grep -Fq "s.paused ? 'на паузі'" \
