...
# `BDO_STATE_DIR` задається В КОМАНДІ, а не через оточення тесту.
#
# tmux-сервер довгоживучий і несе оточення ТОГО, ХТО ЙОГО ПІДНЯВ. Сервер
# інтерфейсу запускає роботу з власним `BDO_STATE_DIR`, і після цього будь-яка
...
out="$(screen)"
grep -qF 'чекає на перевірку якості' <<<"$out" \
