...
// Режим теж мусить мати підпис: із нього складається і ціль прогону, і
// пояснення «що зараз станеться» на екрані старту.
$modes = Labels::missingModes();
if ($modes !== []) {
    fwrite(STDERR, "Режими без українського підпису: ".implode(", ", $modes)."\n");
    exit(1);
}
...
# 14. КОЖНА команда, яку вікно кличе, мусить існувати в маршрутизаторі.
#
#     «Продовжити незавершену пачку» кликало `./bdo run-loop` · такого маршруту
#     немає ні в `Router`, ні в старому bash-вході (там був `loop`). Власник
#     підтверджував «довести до кінця?» і діставав «невідома команда» з кодом 2,
#     тобто пункт меню не працював узагалі. Тринадцять попередніх перевірок
#     цього не бачили за побудовою: вони ганяють ЕКРАНИ на підробленому `./bdo`,
#     який відповідає на будь-яке слово, тому неіснуючого маршруту не існує і
#     для них.
#
#     Перевіряється КЛАС, а не один рядок: кожне літеральне слово після `"$BDO"`
#     звіряється з таблицею маршрутизатора.
while IFS= read -r cmd; do
    test -n "$cmd" || continue
    php -r '
require $argv[1];
$target = Bdo\Translate\Cli\Router::targetFor([$argv[2]]);
if ($target === null) {
    fwrite(STDERR, "Вікно кличе неіснуючу команду: ./bdo {$argv[2]}\n");
    exit(1);
}
