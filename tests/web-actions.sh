...
$cases = [
    ...
    ["models.lock", ["runtime" => "ollama", "model" => "qwen3.6:35b-a3b-mtp-q4_K_M"]],
    ["models.unlock", ["runtime" => "ollama", "model" => "qwen3.6:35b-a3b-mtp-q4_K_M"]],
    ["models.clear", []],
...
foreach (Actions::names() as $name) {
    $payload = match ($name) {
        ...
        "models.unload" => ["runtime" => "omlx", "model" => "model-a"],
        "models.probe" => ["runtime" => "ollama", "model" => "model-a"],
        "models.lock" => ["runtime" => "ollama", "model" => "model-a"],
        "models.unlock" => ["runtime" => "ollama", "model" => "model-a"],
...
grep -Fq 'id="scopeAll"' "$ROOT/web/start.html" \
    || fail 'на сторінці старту немає перемикача повного прогону · власник знову вгадуватиме кількість пачок'
# Поле кількості мусить ЛИШИТИСЬ на місці: перемикач його доповнює, а не
# заміняє. Перша редакція зробила з двох речей одну випадайку, і власник сказав
# прямо, що так незручно.
grep -Fq 'id="batches" type="number"' "$ROOT/web/start.html" \
    || fail 'поле кількості пачок зникло зі сторінки'
