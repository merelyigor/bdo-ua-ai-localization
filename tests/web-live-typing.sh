...
// Очікуємо СИРИЙ вивід моделі · рішення власника 2026-09-17. Раніше тут стояла
// вижимка `B.readable`, і тест стеріг саме її: на екрані був охайний текст, якого
// модель не друкувала. Тепер показане мусить збігатися з тим, що прийшло.
const wantText = B.prettyIfJson(fullRaw);
...
// СИРИЙ ДРУК · рішення власника 2026-09-17. Вимогу «не більше 60 символів за
// кадр» знято свідомо: вона описувала ЗГЛАДЖУВАННЯ, від якого власник
// відмовився · «друкуй як віддає модель, без додумування й анімування».
// Натомість стережемо те, що лишилось важливим від D83: показане мусить
// зростати разом із потоком і ЖОДНОГО разу не зменшитись (це перевіряє
// `onPaint` вище) та збігатися з відповіддю моделі побайтово (перевірено
// вище). Плюс екран мусить оновлюватись НЕ один раз у кінці, інакше друку
// не видно взагалі.
const worst = Math.max.apply(null, paints);
if (paints.length < 2) {
    die('екран оновився ' + paints.length + ' раз · друку не видно, текст зʼявився разом');
}
if (worst > wantText.length) {
    die('один кадр показав більше, ніж надіслала модель · показ вигадує текст');
}
...
grep -Fq 'buffer.sync(prettyIfJson(raw), true)' "$ROOT/web/app.js" \
    || fail 'відповідь друкується не сиро · повернулось згладжування, якого власник просив позбутись'
