...
# МОДЕЛЬ ОДНА, І ВОНА ВИДНА ОДИН РАЗ · рішення власника 2026-09-23. Окремого
# блоку моделей для ролей на екрані бути не повинно: він повторював те, що вже
# написано в «обраній моделі», рівно стільки разів, скільки ролей.
grep -Fq 'role-models-panel' "$MODELS" && fail 'блок моделей ролей повернувся на екран'
grep -Fq 'selection-roles' "$MODELS" && fail 'на екрані лишився перелік моделей для ролей'
grep -Fq 'обрана модель' "$MODELS" || fail 'екран не показує обраної моделі'
