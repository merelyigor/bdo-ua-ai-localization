...
# ПОРЯДОК У ШАПЦІ Є ПРІОРИТЕТОМ ПЕРЕНОСУ. Шапка переноситься по словах, і вниз
# іде ОСТАННІЙ елемент. Поки параметри стояли перед станом звʼязку, на вужчому
# вікні вниз падав САМЕ стан і висів сам-один під брендом (власник 2026-09-18).
# САБОТАЖ: поміняти два рядки місцями · перевірка червоніє.
# Кожен рядок шукаємо ОКРЕМИМ шаблоном. Перша редакція брала обидва збіги одним
# grep-ом і порівнювала «перший» із «другим» · таке порівняння істинне завжди,
# бо перший збіг за визначенням раніший. Саботаж її не завалив, і вона нічого
# не стерегла.
nav_block="$(sed -n '/function renderNav(/,/^  }/p' "$APP")"
sp_line="$(grep -n 'class=\"sp\"' <<<"$nav_block" | head -1 | cut -d: -f1)"
model_line="$(grep -n 'class=\"nav-model\"' <<<"$nav_block" | head -1 | cut -d: -f1)"
test -n "$sp_line" && test -n "$model_line" \
    || fail 'у навігації немає стану звʼязку або параметрів моделі'
test "$sp_line" -lt "$model_line" \
    || fail 'параметри моделі стоять перед станом звʼязку · при переносі вниз падатиме стан, а не параметри'
