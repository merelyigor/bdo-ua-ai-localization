...
#              правка на сайті: за наявної ролі схвалюється одразу.
#   proposal - те саме, але auto_approve=false: рядок лишається в черзі
#              модерації. Це заміна файлового карантину - недосконалий переклад
...
' "$SCRIPT_DIR/lib/autoload.php" "$OUT" "$TIMESTAMP" "$BDO_API_ENV" "$CHANNEL" "${BDO_STATE_DIR:-$SCRIPT_DIR/state}/write-log.jsonl"
